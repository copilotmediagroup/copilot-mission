...
import { publishGuardLocation } from './modules/location/liveLocationRepository'
...
function GuardApp({ developerMode, onEnableDeveloperMode }: { developerMode: boolean; onEnableDeveloperMode: () => void }) {
  ...
  useEffect(() => {
    if (!liveDispatch || mission.state === 'offline') return
    ...
    const watchId = navigator.geolocation.watchPosition(
      (position) => {
        const now = Date.now()
        if (now - lastLocationWrite.current < 8000) return
        lastLocationWrite.current = now
        const trackableStatuses = new Set(['offered', 'accepted', 'en_route', 'arrived', 'active'])
        const jobId = dispatchMission && trackableStatuses.has(dispatchMission.status) ? dispatchMission.job_id : null
        void publishGuardLocation({
          ...
          jobId,
        }).then(() => { locationErrorShown.current = false }).catch((error) => {
          if (!locationErrorShown.current) {
            locationErrorShown.current = true
            setNotice(error instanceof Error ? error.message : 'Unable to publish live location')
          }
        })
      },
      (error) => {
        if (!locationErrorShown.current) {
          locationErrorShown.current = true
          setNotice(error.code === error.PERMISSION_DENIED ? 'Location permission is required while online' : 'Waiting for a reliable GPS signal')
        }
      },
  ...
  }, [liveDispatch, mission.state, dispatchMission?.job_id, dispatchMission?.status])
