-- Co Pilot Security Marketplace OS — Agency Engine Extension: Guard Onboarding
...
begin
  ...
  values(v_agency_id,lower(trim(p_email)),trim(p_full_name),nullif(trim(p_phone),''),nullif(trim(p_badge_number),''),encode(digest(v_token,'sha256'),'hex'),auth.uid()) returning * into v_invite;
...
begin
  ...
  if v_token is not null then
    ...
    where token_hash=encode(digest(v_token,'sha256'),'hex') and status='pending' and expires_at>now() and lower(email)=lower(new.email)
    for update;
