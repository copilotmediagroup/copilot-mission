...
begin
 select am.agency_id into v_agency from public.agency_members am join public.agencies a on a.id=am.agency_id where am.user_id=auth.uid() and am.role='agency_admin' and am.status='active' and a.status='approved' limit 1;
