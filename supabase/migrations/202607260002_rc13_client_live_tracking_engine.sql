-- Co Pilot Security Marketplace OS — RC1.3 Client Live Tracking Engine
-- Read-only, client-scoped projection over Marketplace, Mission, Live Location, and Reporting authorities.
begin;
...
begin
  ...
  select jsonb_build_object(
    ...
    'mission',jsonb_build_object(
      'state',coalesce(me.state,case when j.status='open' then 'marketplace' else j.status::text end),
  ...
  order by
    case when coalesce(me.state,'marketplace') in ('completed','cancelled') then 1 else 0 end,
    j.updated_at desc
  limit 1;
