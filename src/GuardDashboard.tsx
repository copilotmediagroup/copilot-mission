...
function Patrol({ count, next, records, onRecordsChange, incidents, onIncidentsChange, missionStartedAt }: { count: number; next: () => void; records: PatrolEvidence[]; onRecordsChange: (records: PatrolEvidence[]) => void; incidents: IncidentRecord[]; onIncidentsChange: (records: IncidentRecord[]) => void; missionStartedAt?: number | null }) {
  ...
  const gpsReady = true
  const completionReady = missing.length === 0 && guardianReady && gpsReady
  const smartAlert = highSeverity ? 'High-priority incident attached. Guardian remains active.' : warning || (!completionReady ? `Complete ${missing.join(', ')} before moving on.` : current.smartReminder)
  ...
  return <PhoneShell><AppHeader title="MISSION COMMAND"/><main className="screen-content compact-content patrol-content">
    ...
    <section className="mission-live-grid">
      ...
      <div className="live-stat"><small>EVIDENCE</small><strong>{totals.photos + totals.videos + totals.notes}</strong><span>{totals.photos} photos · {totals.videos} videos · {totals.notes} notes</span></div>
    ...
    <section className="completion-readiness"><div className="evidence-heading"><strong>Ready to complete?</strong><span>{completionReady ? 'All checks passed' : `${missing.length} requirement${missing.length === 1 ? '' : 's'} open`}</span></div><div className="readiness-list"><span className={gpsReady ? 'ready' : ''}><CheckCircle2/> GPS within checkpoint radius</span><span className={guardianReady ? 'ready' : ''}><CheckCircle2/> Guardian connection secure</span><span className={missing.length === 0 ? 'ready' : ''}><CheckCircle2/> Required evidence attached</span><span className={checkpointIncidents.length === 0 ? 'ready' : ''}><CheckCircle2/> No incident draft pending</span></div></section>
