...
import { Activity, AlertTriangle, BatteryMedium, Building2, Camera, Check, CheckCircle2, ChevronRight, Circle, ClipboardCheck, Copy, Eye, FileText, Flame, Image, Lightbulb, MapPin, MessageCircle, Navigation, Phone, Power, RadioTower, RefreshCw, RotateCcw, ShieldAlert, ShieldCheck, Trash2, UserRound, Video, Waves, Wifi, X } from 'lucide-react'
import MissionReport from './MissionReport'
import type { IncidentRecord, IncidentSeverity, MissionState, PatrolEvidence } from './types'
import type { DispatchMission, GuardDailySummary } from './modules/dispatch/dispatchRepository'
...
export interface GuardDashboardProps {
  ...
  dailySummary?: GuardDailySummary
}
...
function PropertyHeader({ eyebrow, assignment }: { eyebrow: string; assignment?: DispatchMission | null }) { const name=assignment?.property.name??'Assigned property'; const address=assignment?.property.address??'Address unavailable'; return <><div className="eyebrow">{eyebrow}</div><h2 className="property-title">{name}</h2><p className="address">{address}</p></> }
function PropertyVisual({assignment}:{assignment?:DispatchMission|null}){const url=assignment?.property.photo_url;return url?<img className="property-image" src={url} alt={assignment?.property.name??'Property'}/>:<div className="property-image property-image-empty"><Building2/><strong>{assignment?.property.name??'Assigned property'}</strong><span>No client property photo uploaded</span></div>}

function formatDuty(seconds:number){const hours=Math.floor(seconds/3600);const minutes=Math.floor((seconds%3600)/60);return `${hours}h ${String(minutes).padStart(2,'0')}m`}
function Offline({ next, summary }: { next: () => void; summary:GuardDailySummary }) { return <PhoneShell light><AppHeader light/><main className="screen-content offline-screen"><ProfileBlock/><section className="offline-hero"><h3>You are currently offline</h3><p>Go online to receive<br/>assignments.</p><div className="shield-orbit"><ShieldCheck/></div><PrimaryButton onClick={next}><Power/> GO ONLINE</PrimaryButton></section><h4 className="section-label">Today at a glance</h4><div className="metric-grid"><Metric value={String(summary.jobsToday)} label="Jobs Today"/><Metric value={formatDuty(summary.onDutySeconds)} label="On Duty"/><Metric value={String(summary.checkInsToday)} label="Check-ins"/></div><div className="message-card"><MessageCircle/><div><strong>Messages</strong><small>No unread messages</small></div><ChevronRight/></div></main><BottomNav light/></PhoneShell> }
function Waiting({ offline, summary }: { offline: () => void; summary:GuardDailySummary }) { return <PhoneShell><AppHeader title="CO PILOT"/><main className="screen-content"><ProfileBlock online/><section className="waiting-hero"><div className="radar"><div className="radar-sweep"/></div><h3>You're online and available</h3><p>We'll notify you when a new<br/>assignment is available.</p><PrimaryButton onClick={offline}><Power/> GO OFFLINE</PrimaryButton></section><h4 className="section-label">Today at a glance</h4><div className="metric-grid"><Metric value={String(summary.jobsToday)} label="Jobs Today"/><Metric value={formatDuty(summary.onDutySeconds)} label="On Duty"/><Metric value={String(summary.checkInsToday)} label="Check-ins" accent/></div><div className="message-card dark"><MessageCircle/><div><strong>Messages</strong><small>No unread messages</small></div><ChevronRight/></div></main><BottomNav/></PhoneShell> }
function Assignment({ accept, decline, assignment }: { accept: () => void; decline: () => void; assignment?: DispatchMission | null }) { return <PhoneShell><AppHeader title="NEW ASSIGNMENT"/><main className="screen-content compact-content"><PropertyHeader eyebrow="NEW ASSIGNMENT" assignment={assignment}/><PropertyVisual assignment={assignment}/><div className="two-stats"><div><small>DISTANCE</small><strong>4.2 mi</strong></div><div><small>ETA</small><strong>9 min</strong></div></div><div className="info-grid"><div><small>PATROL TYPE</small><strong>{assignment?.title??'Retail Patrol'}</strong></div><div><small>PRIORITY</small><span className="priority">{assignment?.priority?.toUpperCase()??'MEDIUM'}</span></div></div><div className="payment-row"><small>ASSIGNMENT</small><strong>Marketplace Job</strong></div><SecondaryButton onClick={decline}>DECLINE</SecondaryButton><PrimaryButton onClick={accept}><Check/> ACCEPT</PrimaryButton></main><BottomNav/></PhoneShell> }
function EnRoute({next,assignment}:{next:()=>void;assignment?:DispatchMission|null}){const [metrics,setMetrics]=useState<{distanceText:string;durationText:string}|null>(null);const property=assignment?.property??{name:'Assigned property',address:'Address unavailable',latitude:null,longitude:null};return <PhoneShell><AppHeader title="EN ROUTE"/><main className="screen-content compact-content guard-route-screen"><PropertyHeader eyebrow="EN ROUTE"/><UnifiedMissionMap role="guard" useDeviceLocation property={property} guard={{name:assignment?.guard?.name??'Guard',latitude:null,longitude:null}} onRouteMetrics={setMetrics}/><div className="two-stats"><div><small>ETA</small><strong>{metrics?.durationText??'—'}</strong></div><div><small>DISTANCE</small><strong>{metrics?.distanceText??'—'}</strong></div></div><PrimaryButton onClick={next}><Navigation/> START ROUTE</PrimaryButton></main><BottomNav/></PhoneShell>}
function Arrived({ next, assignment }: { next: () => void; assignment?:DispatchMission|null }) { return <PhoneShell><AppHeader title="ON SITE"/><main className="screen-content compact-content"><PropertyHeader eyebrow="CONFIRM ARRIVAL" assignment={assignment}/><PropertyVisual assignment={assignment}/><div className="arrival-ready-card"><CheckCircle2/><span><strong>Ready to begin patrol</strong><small>Confirm you are on site. Starting patrol creates the official arrival timestamp.</small></span></div><PrimaryButton tone="green" onClick={next}><ClipboardCheck/> START PATROL</PrimaryButton><section className="property-info"><small>PROPERTY INFO</small><div><strong>Maria Contact</strong><Phone/></div><div><strong>Gate / Entry Code<br/><span>#4826</span></strong><Copy/></div><div><strong>Special Instructions<br/><span>Check back entrance and loading dock.</span></strong><ChevronRight/></div></section><SecondaryButton>VIEW DETAILS</SecondaryButton></main><BottomNav/></PhoneShell> }
...
function Patrol({ count, next, records, onRecordsChange, incidents, onIncidentsChange, missionStartedAt, assignment, saving=false }: { count: number; next: () => void; records: PatrolEvidence[]; onRecordsChange: (records: PatrolEvidence[]) => void; incidents: IncidentRecord[]; onIncidentsChange: (records: IncidentRecord[]) => void; missionStartedAt?: number | null; assignment?: DispatchMission | null; saving?: boolean }) {
  ...
  return <PhoneShell><AppHeader title="MISSION COMMAND"/><main className="screen-content compact-content patrol-content">
    ...
    <section className="mission-intelligence-strip">
      <div><small>MISSION</small><strong>{assignment?.title??'Security Patrol'}</strong><span>{assignment?.property.name??'Assigned property'}</span></div>
    ...
    <PrimaryButton tone="orange" onClick={complete} disabled={!completionReady||saving}><Check/> {saving ? 'SAVING CHECKPOINT…' : completionReady ? 'COMPLETE CHECKPOINT' : 'COMPLETE REQUIREMENTS'}</PrimaryButton>
  </main><BottomNav/>
...
function Review({ next, records, incidents, missionStartedAt }: { next: () => void; records: PatrolEvidence[]; incidents: IncidentRecord[]; missionStartedAt?: number | null }) {
  ...
  const [elapsed,setElapsed]=useState(()=>missionStartedAt?Math.max(0,Math.floor((Date.now()-missionStartedAt)/1000)):0)
  useEffect(()=>{if(!missionStartedAt)return;const tick=()=>setElapsed(Math.max(0,Math.floor((Date.now()-missionStartedAt)/1000)));tick();const timer=window.setInterval(tick,1000);return()=>window.clearInterval(timer)},[missionStartedAt])
  return <PhoneShell><AppHeader title="REVIEW & SUBMIT"/><main className="screen-content compact-content review-content"><div className="review-intro"><CheckCircle2/><div><h2>Patrol complete</h2><p>Review the mission record before submitting.</p></div></div><div className="review-summary"><div><small>TIME ON SITE</small><strong>{formatElapsed(elapsed)}</strong></div><div><small>CHECKPOINTS</small><strong>6 of 6</strong></div><div><small>EVIDENCE</small><strong>{totalEvidence} items</strong></div></div><h4 className="section-label">PATROL LOCATIONS</h4><div className="review-list">{checkpoints.map((checkpoint,index)=>{
...
function Completed({ next, incidents, records, missionStartedAt, missionCompletedAt, assignment }: { next: () => void; incidents: IncidentRecord[]; records: PatrolEvidence[]; missionStartedAt?: number | null; missionCompletedAt?: number | null; assignment?: DispatchMission | null }) {
  const [reportOpen, setReportOpen] = useState(false)
  const totalEvidence = records.reduce((sum,item)=>sum+item.photos+item.videos+(item.note?1:0),0)
  const elapsed=missionStartedAt?Math.max(0,Math.floor(((missionCompletedAt??Date.now())-missionStartedAt)/1000)):0
  return <PhoneShell><AppHeader title="MISSION COMPLETE"/><main className="screen-content completed-screen command-complete"><div className="completion-kicker"><ShieldCheck/> MISSION SECURED</div><h2 className="property-title">{assignment?.property.name??'Assigned property'}</h2><p>The professional mission report is ready for agency review and client delivery.</p><div className="success-orbit"><Check/></div><div className="completion-processing report-ready"><span><CheckCircle2/> Evidence synchronized</span><span><CheckCircle2/> Timeline secured</span><span><CheckCircle2/> Report ready</span></div><div className="summary-grid"><div><small>TIME ON SITE</small><strong>{formatElapsed(elapsed)}</strong></div><div><small>CHECKPOINTS</small><strong>6 of 6</strong></div><div><small>EVIDENCE</small><strong>{totalEvidence} Items</strong></div><div><small>INCIDENTS</small><strong>{incidents.length}</strong></div></div><PrimaryButton tone="purple" onClick={()=>setReportOpen(true)}><Eye/> VIEW MISSION REPORT</PrimaryButton><SecondaryButton onClick={next}><RefreshCw/> RETURN ONLINE</SecondaryButton></main><BottomNav/>{reportOpen&&<MissionReport records={records} incidents={incidents} missionStartedAt={missionStartedAt} onClose={()=>setReportOpen(false)}/>}</PhoneShell>
}

export default function GuardDashboard(props: GuardDashboardProps) {
  const { state, assignment, checkpoint=0, onAdvance, patrolEvidence=[], onEvidenceChange=()=>undefined, incidents=[], onIncidentsChange=()=>undefined, missionStartedAt=null, missionCompletedAt=null, checkpointSaving=false, dailySummary={jobsToday:0,onDutySeconds:0,checkInsToday:0} } = props
  if (state === 'offline') return <Offline summary={dailySummary} next={action(props.onGoOnline,onAdvance)}/>
  if (state === 'waiting') return <Waiting summary={dailySummary} offline={action(props.onGoOffline,onAdvance)}/>
  ...
  if (state === 'enroute') return <EnRoute assignment={assignment} next={action(props.onStartRoute,onAdvance)}/>
  if (state === 'arrived') return <Arrived assignment={assignment} next={action(props.onMarkArrived,onAdvance)}/>
  if (state === 'patrol') return <Patrol count={checkpoint} next={action(props.onNextCheckpoint,onAdvance)} records={patrolEvidence} onRecordsChange={onEvidenceChange} incidents={incidents} onIncidentsChange={onIncidentsChange} missionStartedAt={missionStartedAt} assignment={assignment} saving={checkpointSaving}/>
  if (state === 'proof') return <Review next={action(props.onSubmitProof,onAdvance)} records={patrolEvidence} incidents={incidents} missionStartedAt={missionStartedAt}/>
  return <Completed next={action(props.onReturnOnline,onAdvance)} incidents={incidents} records={patrolEvidence} missionStartedAt={missionStartedAt} missionCompletedAt={missionCompletedAt} assignment={assignment}/>
}
