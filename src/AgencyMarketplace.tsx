...
export default function AgencyMarketplace({developerMode=false,accessMode='live',viewedRole='agency_admin'}:{developerMode?:boolean;accessMode?:DeveloperAccessMode;viewedRole?:AppRole}){
  ...
  return <div className="agency-app premium-agency">
    ...
    <aside className="agency-sidebar premium-sidebar">
      ...
      <div className="sidebar-version">v1.5.4 <span className={`backend-mode ${mode}`}><i/>{mode === 'supabase' ? 'Supabase Connected' : 'Mock Mode'}</span></div>
    </aside>
    ...
    <main className="agency-main premium-main">
      {tab==='marketplace'?<Marketplace jobs={jobs} filtered={filtered} filter={filter} setFilter={setFilter} accept={job=>void accept(job)} available={available} activity={activity} loading={marketplaceLoading}/>:tab==='operations'?<Operations accepted={accepted} onMarketplace={()=>setTab('marketplace')}/>:<Placeholder tab={tab}/>} 
    </main>
  </div>
}
...
function Marketplace({jobs,filtered,filter,setFilter,accept,available,activity,loading}:{jobs:Job[];filtered:Job[];filter:'all'|JobKind;setFilter:(v:'all'|JobKind)=>void;accept:(j:Job)=>void;available:Guard[];activity:Activity[];loading:boolean}){
