...
function Marketplace({jobs,filtered,filter,setFilter,accept,available,allGuards,activity,loading,preview}:{jobs:Job[];filtered:Job[];filter:'all'|JobKind|'my_guards';setFilter:(v:'all'|JobKind|'my_guards')=>void;accept:(j:Job)=>void;available:Guard[];allGuards:Guard[];activity:Activity[];loading:boolean;preview:boolean}){
 ...
 const eligible=useMemo(()=>available.filter(g=>typeof g.latitude==='number'&&typeof g.longitude==='number'),[available])
 const closest=useMemo(()=>{if(!selectedJob||typeof selectedJob.latitude!=='number'||typeof selectedJob.longitude!=='number')return null;return eligible.map(g=>({...g,proximity:haversineMiles(g.latitude!,g.longitude!,selectedJob.latitude!,selectedJob.longitude!)})).sort((a,b)=>a.proximity-b.proximity)[0]??null},[selectedJob,eligible])
 useEffect(()=>{setRouteMetrics(null);setResolvedAddress(closest?.currentAddress??null);if(!closest||closest.currentAddress||typeof closest.latitude!=='number'||typeof closest.longitude!=='number')return;let cancelled=false;(async()=>{try{const google=await loadGoogleMaps();const geocoder=new google.maps.Geocoder();geocoder.geocode({location:{lat:closest.latitude!,lng:closest.longitude!}},(results:any[],status:string)=>{if(!cancelled&&status==='OK')setResolvedAddress(results?.[0]?.formatted_address??null)})}catch{}})();return()=>{cancelled=true}},[closest?.sourceId,closest?.latitude,closest?.longitude,closest?.currentAddress])
 return <div className="premium-dashboard">
  ...
  <section className="live-map-panel premium-panel"><div className="premium-panel-head"><div><strong>LIVE MARKETPLACE MAP</strong><span><i/>LIVE</span></div><button><Layers3/>Layers<ChevronDown/></button></div><div className="map-filter-row">{(['all','standard','priority','emergency'] as const).map(v=><button key={v} className={filter===v?'active':''} onClick={()=>setFilter(v)}>{v==='all'?'All':v==='standard'?'Open Jobs':v[0].toUpperCase()+v.slice(1)}</button>)}<button className={filter==='my_guards'?'active':''} onClick={()=>setFilter('my_guards')}>My Guards</button></div>
   {selectedJob&&closest?<UnifiedMissionMap property={{name:selectedJob.property,address:selectedJob.address,latitude:selectedJob.latitude??null,longitude:selectedJob.longitude??null}} guard={{name:closest.name,latitude:closest.latitude??null,longitude:closest.longitude??null,freshness:closest.freshness}} role="agency" className="marketplace-proximity-map" onRouteMetrics={setRouteMetrics}/>:<AgencyOperationsMap jobs={filtered.map(job=>({id:job.id,title:job.property||job.title,kind:job.kind,latitude:job.latitude??null,longitude:job.longitude??null}))} guards={allGuards.map(guard=>({id:guard.sourceId??String(guard.id),name:guard.name,status:guard.status,latitude:guard.latitude??null,longitude:guard.longitude??null,freshness:guard.freshness??null}))} showJobs={filter!=='my_guards'} showGuards={filter==='all'||filter==='my_guards'}/>} 
   {selectedJob&&<button className="proximity-map-close" onClick={()=>setSelectedJob(null)}><X/>All marketplace markers</button>}
  </section>
  <section className="opportunities premium-panel"><div className="premium-panel-head"><div><strong>{filterLabel.toUpperCase()} <b>{filter==='my_guards'?allGuards.filter(g=>g.status!=='offline').length:filtered.length}</b></strong></div><button>Nearest<ChevronDown/></button></div><div className="premium-job-list">{loading?<div className="marketplace-list-state"><span className="marketplace-state-pulse"/><strong>Synchronizing opportunities</strong><small>Checking the live marketplace…</small></div>:filtered.length?filtered.map(j=><article data-job-id={j.id} key={j.id} className={`premium-job-card ${j.kind} ${selectedJob?.id===j.id?'selected':''}`} onClick={()=>setSelectedJob(j)}>{j.photoUrl?<div className="marketplace-property-photo"><img src={j.photoUrl} alt={`${j.property} property`}/></div>:<div className="marketplace-property-photo placeholder"><Building2/></div>}<div className="job-card-top"><span className={`kind-chip ${j.kind}`}>{j.kind==='emergency'?<Siren/>:j.kind==='priority'?<Zap/>:<BriefcaseBusiness/>}{j.kind==='standard'?'Open Job':j.kind}</span><span>{selectedJob?.id===j.id&&routeMetrics?routeMetrics.distanceText:`${j.distance} mi`}<small>{selectedJob?.id===j.id&&routeMetrics?`ETA ${routeMetrics.durationText}`:`ETA ${j.eta} min`}</small></span></div><h3>{j.property}</h3><p><strong>{j.title}</strong><br/>{j.address}</p><div className="job-card-meta"><span><Building2/>{j.property}</span><span><Clock3/>{j.duration} min</span><span><Users/>1 Guard</span></div><div className="job-card-action"><strong>{j.price>0?`$${j.price}`:'Mission'}</strong><button onClick={e=>{e.stopPropagation();accept(j)}}>Claim Mission</button></div></article>):<div className="marketplace-list-state empty"><BriefcaseBusiness/><strong>{filter==='my_guards'?'Guard layer selected':`No ${filterLabel.toLowerCase()}`}</strong><small>{filter==='my_guards'?'Use the live map to view your online guards.':'New verified missions in this category will appear here in real time.'}</small></div>}</div></section>
  <aside className="right-rail">{selectedJob?<section className="proximity-card premium-panel"><div className="premium-panel-head"><strong>CLOSEST-GUARD RECOMMENDATION</strong><span>LIVE</span></div><div className="proximity-property">{selectedJob.photoUrl?<img src={selectedJob.photoUrl} alt=""/>:<span><Building2/></span>}<div><small>PROPERTY</small><strong>{selectedJob.property}</strong><p>{selectedJob.address}</p></div></div>{closest?<><div className="recommended-guard">{closest.photoUrl?<img src={closest.photoUrl} alt={closest.name}/>:<span>{closest.initials}</span>}<div><small>RECOMMENDED GUARD</small><strong>{closest.name}</strong><p>{resolvedAddress||'Resolving current GPS address…'}</p></div><b>{closest.freshness==='live'?'LIVE GPS':closest.freshness?.toUpperCase()}</b></div><div className="proximity-metrics"><span><small>ROAD DISTANCE</small><strong>{routeMetrics?.distanceText??`${closest.proximity.toFixed(1)} mi`}</strong></span><span><small>DRIVE ETA</small><strong>{routeMetrics?.durationText??'Calculating…'}</strong></span></div><button className="proximity-primary" onClick={()=>accept(selectedJob)}>Claim Mission</button><small className="proximity-note">After claim, this recommendation becomes the assignment recommendation in Operations.</small></>:<div className="marketplace-list-state empty"><LocateFixed/><strong>No eligible live Guard</strong><small>An available Guard with fresh GPS is required for route intelligence.</small></div>}</section>:<><section className="capacity-panel premium-panel"><div className="premium-panel-head"><strong>AGENCY CAPACITY</strong></div><div className="capacity-content"><div className="capacity-ring"><span><strong>{available.length}</strong><small>Available</small></span></div><div className="capacity-breakdown"><span>Total Guards <b>{allGuards.length}</b></span><span>Available <b>{available.length}</b></span><span>On Mission <b>{allGuards.filter(g=>g.status==='on-mission').length}</b></span><span>Reserved <b>{allGuards.filter(g=>g.status==='reserved').length}</b></span><span>Offline <b>{allGuards.filter(g=>g.status==='offline').length}</b></span></div></div></section><section className="activity-panel premium-panel"><div className="premium-panel-head"><strong>MARKETPLACE ACTIVITY</strong><span>{preview?'Preview Feed':'Live Feed'}</span></div>{activity.length?activity.map(a=><div className="activity-row" key={a.id}><span className={a.type==='emergency'?'danger':''}>{a.type==='emergency'?<Siren/>:a.type==='accepted'?<Check/>:a.type==='assigned'?<Users/>:<BriefcaseBusiness/>}</span><small>{a.time}</small><div><strong>{a.title}</strong><p>{a.location}</p></div></div>):<div className="marketplace-list-state empty"><Radio/><strong>Awaiting live activity</strong><small>Mission events will appear here.</small></div>}</section></>}</aside>
...
function haversineMiles(lat1:number,lng1:number,lat2:number,lng2:number){const r=3958.7613,toRad=(v:number)=>v*Math.PI/180;const dLat=toRad(lat2-lat1),dLng=toRad(lng2-lng1);const a=Math.sin(dLat/2)**2+Math.cos(toRad(lat1))*Math.cos(toRad(lat2))*Math.sin(dLng/2)**2;return 2*r*Math.asin(Math.sqrt(a))}
