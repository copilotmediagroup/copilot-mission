...
function Marketplace({jobs,filtered,filter,setFilter,accept,available,allGuards,activity,loading,preview}:{jobs:Job[];filtered:Job[];filter:'all'|JobKind|'my_guards';setFilter:(v:'all'|JobKind|'my_guards')=>void;accept:(j:Job)=>void;available:Guard[];allGuards:Guard[];activity:Activity[];loading:boolean;preview:boolean}){
 ...
 return <div className="premium-dashboard">
  ...
  <section className="live-map-panel premium-panel"><div className="premium-panel-head"><div><strong>FASTEST RESPONSE MAP</strong><span><i/>LIVE ROUTING</span></div><button><Layers3/>Layers<ChevronDown/></button></div><div className="map-filter-row">{(['all','standard','priority','emergency'] as const).map(v=><button key={v} className={filter===v?'active':''} onClick={()=>setFilter(v)}>{v==='all'?'All':v==='standard'?'Open Jobs':v[0].toUpperCase()+v.slice(1)}</button>)}<button className={filter==='my_guards'?'active':''} onClick={()=>setFilter('my_guards')}>My Guards</button></div>
   {selectedJob&&selectedGuard?<UnifiedMissionMap property={{name:selectedJob.property,address:selectedJob.address,latitude:selectedJob.latitude??null,longitude:selectedJob.longitude??null}} guard={{name:selectedGuard.name,latitude:selectedGuard.latitude??null,longitude:selectedGuard.longitude??null,freshness:selectedGuard.freshness}} role="agency" className="marketplace-proximity-map"/>:<AgencyOperationsMap jobs={filtered.map(job=>({id:job.id,title:job.property||job.title,kind:job.kind,latitude:job.latitude??null,longitude:job.longitude??null}))} guards={allGuards.map(guard=>({id:guard.sourceId??String(guard.id),name:guard.name,status:guard.status,latitude:guard.latitude??null,longitude:guard.longitude??null,freshness:guard.freshness??null}))} showJobs={filter!=='my_guards'} showGuards={filter==='all'||filter==='my_guards'}/>} 
