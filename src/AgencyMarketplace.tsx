...
const initialActivity:Activity[] = [
  ...
  {id:2,time:'11:45 AM',type:'accepted',title:'Agency accepted',location:'Vacant Home Patrol'},
...
export default function AgencyMarketplace(){
  ...
  useEffect(()=>{
    if(incomingIndex>=incomingJobs.length) return
    const timer=window.setTimeout(()=>{
      const job=incomingJobs[incomingIndex]
      setJobs(v=>[job,...v])
      setActivity(v=>[{id:Date.now(),time:'Now',type:(job.kind==='emergency'?'emergency':'new') as Activity['type'],title:job.kind==='priority'?'New priority job':'New job posted',location:job.client},...v].slice(0,6))
  ...
  const accept=(job:Job)=>{
    ...
    setAccepted(v=>[job,...v])
    setActivity(v=>[{id:Date.now(),time:'Now',type:'accepted',title:'Alpha Force accepted',location:job.title},...v].slice(0,6))
    setToast(`${job.title} locked to Alpha Force and moved to Operations.`)
  }
  return <div className="agency-app premium-agency">
    ...
    <aside className="agency-sidebar premium-sidebar">
      ...
      <div className="sidebar-version">v1.2.0 <span className={`backend-mode ${mode}`}><i/>{mode === 'supabase' ? 'Supabase Connected' : 'Mock Mode'}</span></div>
    </aside>
    ...
    <main className="agency-main premium-main">
      {tab==='marketplace'?<Marketplace jobs={jobs} filtered={filtered} filter={filter} setFilter={setFilter} accept={accept} available={available} activity={activity}/>:tab==='operations'?<Operations accepted={accepted} onMarketplace={()=>setTab('marketplace')}/>:<Placeholder tab={tab}/>} 
    </main>
  </div>
}
