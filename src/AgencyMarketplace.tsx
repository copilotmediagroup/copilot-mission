...
export default function AgencyMarketplace(){
  ...
  useEffect(()=>{
    ...
    }).catch(error=>setToast(error instanceof Error?error.message:'Unable to load marketplace.'))
    return()=>{active=false}
  },[mode,user?.id])
  ...
  useEffect(()=>{
    if(mode==='supabase'||incomingIndex>=incomingJobs.length) return
  ...
  const accept=async(job:Job)=>{
    ...
    if(mode==='supabase'){
      if(!agencyId){setToast('Agency account is not ready.');return}
    ...
    setActivity(current => [{id:Date.now(),time:'Now',type:'accepted',title:`${agencyName} accepted`,location:job.title},...current].slice(0,6))
  ...
  return <div className="agency-app premium-agency">
    ...
    <aside className="agency-sidebar premium-sidebar">
      ...
      <div className="sidebar-version">v1.5.0 <span className={`backend-mode ${mode}`}><i/>{mode === 'supabase' ? 'Supabase Connected' : 'Mock Mode'}</span></div>
    </aside>
