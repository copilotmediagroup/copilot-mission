...
export default function AgencyMarketplace({developerMode=false,accessMode='live',viewedRole='agency_admin'}:{developerMode?:boolean;accessMode?:DeveloperAccessMode;viewedRole?:AppRole}){
  ...
  const runtimeGuards=isPreview?guards:[]
  ...
  const accept=async(job:Job)=>{
    ...
    if(mode==='supabase'){
      ...
      try{
        ...
        if(!result.accepted){
          const messages:Record<string,string>={already_claimed:'Another agency claimed this mission first.',agency_not_approved:'Your agency must be approved by Platform Admin before claiming missions.',not_authorized:'This account is not an active Agency Admin for this agency.',mission_unavailable:'This mission is no longer available.'}
          if(result.reason==='already_claimed'||result.reason==='mission_unavailable') setJobs(v=>v.filter(j=>j.id!==job.id))
  ...
  return <div className="agency-app premium-agency">
    ...
    <aside className="agency-sidebar premium-sidebar">
      ...
      <div className="agency-mini-card"><div className="mini-agency"><span>AF</span><div><strong>{agencyName}</strong><small><BadgeCheck/>Verified Agency</small></div></div><div className="mini-stats"><span>Total Guards <b>{runtimeGuards.length}</b></span><span>Available <b>{available.length}</b></span><span>On Mission <b>{runtimeGuards.filter(g=>g.status==='on-mission').length}</b></span><span>Reserved <b>{runtimeGuards.filter(g=>g.status==='reserved').length}</b></span></div></div>
      <div className="sidebar-version">v1.5.7.1 <span className={`backend-mode ${mode}`}><i/>{mode === 'supabase' ? 'Supabase Connected' : 'Mock Mode'}</span></div>
    </aside>
    ...
    <header className="agency-topbar premium-topbar">
      ...
      <div className="top-kpis"><Kpi icon={<CircleDollarSign/>} label="OPEN JOBS" value={jobs.length} tone="gold"/><Kpi icon={<Flame/>} label="PRIORITY" value={jobs.filter(j=>j.kind==='priority').length} tone="orange"/><Kpi icon={<Siren/>} label="EMERGENCY" value={jobs.filter(j=>j.kind==='emergency').length} tone="red"/><Kpi icon={<Users/>} label="ACTIVE JOBS" value={accepted.length+(isPreview?2:0)} tone="green"/><Kpi icon={<ShieldCheck/>} label="ONLINE GUARDS" value={runtimeGuards.filter(g=>g.status!=='offline').length} tone="blue"/></div>
...
function Operations({accepted,preview,onMarketplace}:{accepted:Job[];preview:boolean;onMarketplace:()=>void}){const active=preview?[...accepted,{...initialJobs[0],title:'Nightly Retail Patrol'}]:accepted;return <section className="operations-page"><div className="operations-heading"><div><span className="eyebrow">LIVE OPERATIONS</span><h1>Work won. Teams moving.</h1><p>Manage active assignments while marketplace access stays open.</p></div><button onClick={onMarketplace}><Crosshair/>Return to Marketplace</button></div><div className="operations-capacity-banner"><div><Users/><span><strong>{preview?'7 guards remain available':'Live guard capacity'}</strong><small>{preview?'Preview capacity simulation.':'Guard availability will populate from Supabase.'}</small></span></div><button onClick={onMarketplace}>View open jobs<ChevronRight/></button></div><div className="operations-grid">{active.map((j,i)=><article className="operation-card" key={`${j.id}-${i}`}><div className="operation-status"><span className={i===active.length-1?'active':'awaiting'}>{i===active.length-1?'Mission Active':'Awaiting Assignment'}</span><small>JOB #{j.id}</small></div><h3>{j.title}</h3><p><MapPin/>{j.address}</p><div className="operation-progress"><i style={{width:i===active.length-1?'62%':'12%'}}/></div><div className="operation-footer"><div className="guard-avatar">{i===active.length-1?'MR':'—'}</div><div><small>{i===active.length-1?'ASSIGNED GUARD':'NEXT ACTION'}</small><strong>{i===active.length-1?'Marcus Reed':'Assign available guard'}</strong></div><button>{i===active.length-1?'Open Mission':'Assign'}<ChevronRight/></button></div></article>)}</div></section>}
