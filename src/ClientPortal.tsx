...
import { AlertTriangle, Archive, Camera, Bell, Building2, CalendarClock, Check, CheckCircle2, ChevronRight, Clock3, Home, ImageOff, LoaderCircle, LocateFixed, LogOut, MapPin, Menu, Pencil, Plus, Radio, RefreshCw, Search, Shield, ShieldAlert, Sparkles, Trash2, X } from 'lucide-react'
import { useAuth } from './modules/auth/AuthProvider'
import { archiveClientProperty, createClientJob, createClientProperty, deleteClientProperty, getClientWorkspace, subscribeToClientWorkspace, updateClientProperty, type ClientJob, type ClientProperty } from './modules/client/clientRepository'
...
export default function ClientPortal() {
  ...
  return <div className="client-shell">
    ...
    <main className="client-main">
      ...
      <div className="build-badge">v1.5.6 · COMPLETE PROPERTY MANAGEMENT</div>
    </main>

    {propertyOpen && <PropertyModal clientId={clientId} property={editingProperty} onClose={()=>{setPropertyOpen(false);setEditingProperty(null)}} onSaved={async(mode)=>{setPropertyOpen(false);setEditingProperty(null);setNotice(mode==='created'?'Property added successfully.':'Property updated everywhere.');await load()}}/>}
    {confirmAction && <ConfirmPropertyAction action={confirmAction} onClose={()=>setConfirmAction(null)} onConfirmed={async()=>{const action=confirmAction;setConfirmAction(null);try{if(action.type==='archive')await archiveClientProperty(action.property.id);else await deleteClientProperty(action.property.id);setNotice(action.type==='archive'?'Property archived.':'Property permanently deleted.');await load()}catch(cause){setError(cause instanceof Error?cause.message:'Unable to update property.')}}}/>} 
...
function PropertyModal({ clientId, property, onClose, onSaved }: { clientId:string|null; property:ClientProperty|null; onClose:()=>void; onSaved:(mode:'created'|'updated')=>void }) {
  ...
  const [preview,setPreview]=useState(property?.photo_url||'')
  ...
  return <Modal title={editing?'Edit property':'Add property'} eyebrow="VERIFIED COVERAGE LOCATION" onClose={onClose}><form className="client-form" onSubmit={submit}>
    ...
    <label>Property photo<div className="property-photo-upload">{preview&&!removePhoto?<img src={preview} alt="Property preview"/>:<ImageOff/>}<span><b>{editing?'Replace property picture':'Add a clear property picture'}</b><small>Updates flow to Marketplace, Agency, Guard, and Mission Control.</small></span><input type="file" name="photo" accept="image/*" onChange={event=>photoChanged(event.target.files?.[0])}/></div></label>
    {editing&&property?.photo_url&&<button className="remove-photo-button" type="button" onClick={()=>{setRemovePhoto(true);setPreview('')}}><ImageOff/>Remove current photo</button>}
