...
import { AlertTriangle, Bell, Building2, CalendarClock, CheckCircle2, ChevronRight, Clock3, Home, LogOut, MapPin, Menu, Plus, Radio, RefreshCw, Shield, ShieldAlert, Sparkles, X } from 'lucide-react'
...
export default function ClientPortal() {
  ...
  return <div className="client-shell">
    ...
    <main className="client-main">
      ...
      <div className="build-badge">v1.3.0 · CLIENT PORTAL FOUNDATION</div>
    </main>
...
function PropertyModal({ clientId, onClose, onCreated }: { clientId:string|null; onClose:()=>void; onCreated:()=>void }) {
  ...
  const submit=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!clientId)return;setBusy(true);setError('');const data=new FormData(event.currentTarget);try{await createClientProperty({clientId,name:String(data.get('name')),address:String(data.get('address'))});await onCreated()}catch(cause){setError(cause instanceof Error?cause.message:'Unable to add property.')}finally{setBusy(false)}}
  return <Modal title="Add property" eyebrow="NEW COVERAGE LOCATION" onClose={onClose}><form className="client-form" onSubmit={submit}><label>Property name<input name="name" required placeholder="Home, office or site name"/></label><label>Full address<textarea name="address" required placeholder="Street, city, state and ZIP"/></label>{error&&<div className="client-form-error"><AlertTriangle/>{error}</div>}<div className="client-form-actions"><button type="button" onClick={onClose}>Cancel</button><button className="primary" disabled={busy}>{busy?'Saving…':'Save property'}</button></div></form></Modal>
}
