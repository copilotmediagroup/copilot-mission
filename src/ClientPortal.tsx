...
import { AlertTriangle, Camera, Bell, Building2, CalendarClock, Check, CheckCircle2, ChevronRight, Clock3, Home, LoaderCircle, LocateFixed, LogOut, MapPin, Menu, Plus, Radio, RefreshCw, Search, Shield, ShieldAlert, Sparkles, X } from 'lucide-react'
...
export default function ClientPortal() {
  ...
  return <div className="client-shell">
    ...
    <main className="client-main">
      ...
      <div className="build-badge">v1.5.5 · PROPERTY MEDIA + CLAIM RELIABILITY</div>
    </main>
...
function PropertyModal({ clientId, onClose, onCreated }: { clientId:string|null; onClose:()=>void; onCreated:()=>void }) {
  ...
  const submit=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    const photo=(event.currentTarget.elements.namedItem('photo') as HTMLInputElement)?.files?.[0]
    if(!photo){setError('Add a clear picture of the property before saving.');return}
    if(!photo.type.startsWith('image/')){setError('Property photo must be an image file.');return}
    if(photo.size>8*1024*1024){setError('Property photo must be 8 MB or smaller.');return}
    setBusy(true);setError('')
    const data=new FormData(event.currentTarget)
    try{
      await createClientProperty({clientId,name:String(data.get('name')),address:selected.formattedAddress,street:selected.street,city:selected.city,state:selected.state,postalCode:selected.postalCode,latitude:selected.latitude,longitude:selected.longitude,provider:selected.provider,providerId:selected.providerId,photo})
      await onCreated()
  ...
  return <Modal title="Add property" eyebrow="VERIFIED COVERAGE LOCATION" onClose={onClose}><form className="client-form" onSubmit={submit}>
    <label>Property name<input name="name" required placeholder="Home, office or site name"/></label><label>Property photo<div className="property-photo-upload"><Camera/><span><b>Add a clear property picture</b><small>Shown to agencies, guards, and Mission Control. JPG, PNG, or WEBP up to 8 MB.</small></span><input type="file" name="photo" accept="image/*" required/></div></label>
    ...
    {selected&&<><div className="verified-address-card"><div><LocateFixed/></div><span><small>VERIFIED MAP LOCATION</small><b>{selected.formattedAddress}</b><em>{selected.latitude.toFixed(5)}, {selected.longitude.toFixed(5)}</em></span><CheckCircle2/></div><div className="address-map-confirm"><iframe title="Confirm property map location" loading="lazy" referrerPolicy="no-referrer-when-downgrade" src={`https://www.google.com/maps?q=${selected.latitude},${selected.longitude}&z=18&output=embed`}/><div><LocateFixed/><span><b>Confirm the pin</b><small>Make sure the marker is on the correct property before saving.</small></span></div></div></>}
