...
import { searchVerifiedAddresses, type VerifiedAddress } from './modules/location/addressSearch'
...
export default function ClientPortal() {
  ...
  return <div className="client-shell">
    ...
    <main className="client-main">
      ...
      <div className="build-badge">v1.3.1 · VERIFIED ADDRESS LOCATION</div>
    </main>
...
function PropertyModal({ clientId, onClose, onCreated }: { clientId:string|null; onClose:()=>void; onCreated:()=>void }) {
  ...
  const [results,setResults]=useState<VerifiedAddress[]>([])
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(async()=>{
      ...
      try{setResults(await searchVerifiedAddresses(query,controller.signal))}
  ...
  const choose=(address:VerifiedAddress)=>{setSelected(address);setQuery(address.formattedAddress);setResults([]);setSearchError('')}
  ...
  return <Modal title="Add property" eyebrow="VERIFIED COVERAGE LOCATION" onClose={onClose}><form className="client-form" onSubmit={submit}>
    ...
    {results.length>0&&<div className="address-results" role="listbox">{results.map(result=><button type="button" key={`${result.providerId}-${result.latitude}`} onClick={()=>choose(result)}><MapPin/><span><b>{result.street||result.formattedAddress}</b><small>{[result.city,result.state,result.postalCode,result.country].filter(Boolean).join(', ')}</small></span><ChevronRight/></button>)}</div>}
