...
export default function AgencyOperationsMap({jobs,guards,showJobs,showGuards}:AgencyOperationsMapProps){
 ...
 useEffect(()=>{const google=window.google,map=mapRef.current;if(!google?.maps||!map)return
  ...
  points.forEach(point=>{const position={lat:point.lat,lng:point.lng};bounds.extend(position)
   const isGuard=point.type==='guard';const item:any=point.item
   const marker=new google.maps.Marker({map,position,title:isGuard?item.name:item.title,icon:{url:isGuard?icons.guard:icons[item.kind],scaledSize:new google.maps.Size(isGuard?44:42,isGuard?44:42),anchor:new google.maps.Point(isGuard?22:21,isGuard?22:21)},zIndex:isGuard?5:3})
