...
export type MissionAction =
  ...
  | { type: 'SUBMIT_PROOF' }
  | { type: 'RETURN_ONLINE' }
  | { type: 'HYDRATE_LIVE_STATE'; state: MissionState; missionStartedAt?: number | null }
...
export function missionReducer(snapshot: MissionSnapshot, action: MissionAction): MissionSnapshot {
  switch (action.type) {
    ...
    case 'HYDRATE_LIVE_STATE': return { ...snapshot, state: action.state, missionStartedAt: action.missionStartedAt ?? snapshot.missionStartedAt }
