...
function AuthScreen() {
  const { signIn, signUp, activateGuard } = useAuth()
  const inviteToken=new URLSearchParams(window.location.search).get('guard-invite'); const inviteEmail=new URLSearchParams(window.location.search).get('email')??'';
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (inviteToken) { const result=await activateGuard({email:String(form.get('email')),password:String(form.get('password')),inviteToken}); setMessage(result.error?.message ?? (result.needsEmailConfirmation?'Check your email to confirm and activate your Guard account.':'Guard account activated.')) }
    else if (view === 'login') { const { error } = await signIn(String(form.get('email')), String(form.get('password'))); if (error) setMessage(error.message) }
  ...
  return <div className="auth-page">
    ...
    <section className="auth-panel"><div className="auth-form-wrap">
      ...
      <span className="auth-kicker">SECURE ACCESS</span><h2>{inviteToken ? 'Activate Guard account' : view === 'login' ? 'Welcome back' : view === 'signup' ? 'Create your account' : 'Reset your password'}</h2><p className="auth-subtitle">{inviteToken ? 'Your Agency invited you to join its verified Guard roster.' : view === 'login' ? 'Sign in to continue to your protected workspace.' : view === 'signup' ? 'Choose how you will use the marketplace.' : 'Enter your account email to continue.'}</p>
      {!inviteToken && view === 'signup' && <div className="auth-account-types"><button type="button" className={accountType==='client'?'active':''} onClick={()=>setAccountType('client')}><UserRound/><b>Client</b><small>Request security</small></button><button type="button" className={accountType==='agency_admin'?'active':''} onClick={()=>setAccountType('agency_admin')}><Building2/><b>Agency</b><small>Provide coverage</small></button></div>}
      <form onSubmit={submit}>
        {!inviteToken && view === 'signup' && <label>Full name<div className="auth-input"><UserRound/><input name="fullName" required autoComplete="name" placeholder="Your full name"/></div></label>}
        {!inviteToken && view === 'signup' && accountType === 'agency_admin' && <label>Agency name<div className="auth-input"><Building2/><input name="agencyName" required placeholder="Legal agency name"/></div></label>}
        <label>Email address<div className="auth-input"><Mail/><input name="email" type="email" required autoComplete="email" placeholder="name@company.com" defaultValue={inviteToken?inviteEmail:undefined} readOnly={Boolean(inviteToken)}/></div></label>
        ...
        {!inviteToken && view === 'login' && <div className="auth-options"><label className="auth-remember"><input type="checkbox"/>Remember me</label><button type="button" onClick={()=>{setView('forgot');setMessage('')}}>Forgot password?</button></div>}
        {message && <div className="auth-message"><CheckCircle2/>{message}</div>}
        <button className="auth-submit" disabled={busy}>{busy?<LoaderCircle className="spin"/>:null}{inviteToken?'Activate Guard account':view==='login'?'Sign in':view==='signup'?'Create account':'Continue'}</button>
      </form>
      {!inviteToken && view === 'login' && <div className="auth-create"><span>New to Co Pilot?</span><button onClick={()=>{setView('signup');setMessage('')}}>Create an account</button></div>}
