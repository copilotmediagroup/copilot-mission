...
export async function getClientWorkspace(userId:string) {
  ...
  const [{data:properties,error:propertiesError},{data:jobs,error:jobsError}]=await Promise.all([
    db.from('properties').select('id,client_id,name,address,street,city,state,postal_code,formatted_address,latitude,longitude,geocoding_provider,geocoding_place_id,created_at').eq('client_id',client.id).order('created_at',{ascending:false}),
...
export async function createClientProperty(input:{clientId:string;name:string;address:string;street:string;city:string;state:string;postalCode:string;latitude:number;longitude:number;provider:string;providerId:string}) {
  const db=requireSupabase()
  const payload={
