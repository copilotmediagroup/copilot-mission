...
export async function publishGuardLocation(position:GeolocationPosition):Promise<PublishedLocation>{
  const {latitude,longitude,accuracy,heading,speed}=position.coords
  const {data,error}=await db().rpc('publish_guard_location_rc12',{
    p_latitude:latitude,p_longitude:longitude,p_accuracy_meters:Number.isFinite(accuracy)?accuracy:null,
    p_heading_degrees:heading!=null&&Number.isFinite(heading)?heading:null,p_speed_mps:speed!=null&&Number.isFinite(speed)?speed:null,
  })
  if(error)throw new Error(error.message)
  return data as PublishedLocation
}

export async function getAgencyLiveLocations():Promise<GuardLiveLocation[]>{const {data,error}=await db().rpc('get_agency_live_locations_rc12');if(error)throw new Error(error.message);return Array.isArray(data)?data as GuardLiveLocation[]:[]}
