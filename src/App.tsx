...
import { getGuardDispatchWorkspace, getGuardPresence, respondToAssignment, setGuardPresence, subscribeToDispatch, type DispatchMission } from './modules/dispatch/dispatchRepository'
...
function PortalPlaceholder({ title, body, onLogout }: { title: string; body: string; onLogout: () => void }) {
  return <div className="auth-state"><div className="auth-state-card"><div className="auth-state-icon"><ShieldCheck/></div><h1>{title}</h1><p>{body}</p><button onClick={onLogout}>Log out</button><div className="build-badge">RC2.1 · STATE RECOVERY</div></div></div>
}
...
function GuardApp({ developerMode, onEnableDeveloperMode }: { developerMode: boolean; onEnableDeveloperMode: () => void }) {
  ...
  const loadDispatch = useCallback(async () => {
    ...
    try {
      ...
      } else if (assignment.status === 'accepted' || assignment.status === 'en_route') {
  ...
  return <GuardianProvider missionState={mission.state}><div className={`guard-app state-${mission.state}`}>
    ...
    <div className="build-badge">RC2.1 · STATE RECOVERY</div>
