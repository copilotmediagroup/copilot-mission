...
export default function App() {
  const [portal, setPortal] = useState<'agency' | 'guard'>('agency')
  if (isDeveloperRoute) return <ExperienceLab />
  return <div className="portal-root">
    <div className="portal-switcher">
      <button className={portal === 'agency' ? 'active' : ''} onClick={() => setPortal('agency')}><ShieldCheck/>Agency Marketplace</button>
      <button className={portal === 'guard' ? 'active' : ''} onClick={() => setPortal('guard')}><UserRound/>Guard Mission</button>
    </div>
    {portal === 'agency' ? <AgencyMarketplace /> : <GuardApp />}
  </div>
}
...
function GuardApp() {
  ...
  return <GuardianProvider missionState={mission.state}><div className={`guard-app state-${mission.state}`}>
    ...
    <div className="build-badge">v1.1.0.2 · MARKETPLACE ARCHITECTURE + REPORT FIX</div>
